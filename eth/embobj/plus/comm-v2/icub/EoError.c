...
{   // very important: fill table with order of eOerror_value_SYS_t
    ...
    {eoerror_value_SYS_tobedecided,             "SYS: tbd code. We must decide how to manage this code. it is wip", ""},  
    ...
    {eoerror_value_SYS_canservices_rxfifooverflow, "SYS: the HAL could not put a CAN frame in rx fifo.", "In par16 there is: on msb the rx fifo size, in lsb a code."}, //NOT USED in FW 
    ...
    {eoerror_value_SYS_proxy_reply_fails,        "SYS: the proxy fails to forward a request.", "in par64 there are the [sign|id32], in par16 [capacity|size] of proxy list. if par16 is 0, then the rop could not be tx."}, //NOT USED  
    ...
    {eoerror_value_SYS_canservices_boards_searched, "SYS: the can discovery service searched the following boards on the specified CAN bus.", "In par64 there are the searched addresses represented as nibbles."},    //NOT USED
    {eoerror_value_SYS_canservices_boards_found, "SYS: the can discovery service found the following boards on the specified CAN bus.", "In par64 there are the found addresses represented as nibbles."}, //NOT USED
    ...
    {eoerror_value_SYS_canservices_board_detected, "SYS: the can discovery service found the board", "on the specified CAN bus addr. In par64 there is the type, fw and prot version."},
    {eoerror_value_SYS_canservices_board_wrongprotversion, "SYS: the board has incompatible fw protocol version.", "In par16 there is required prot, in par64 there is detected type, fw and prot version."},
    {eoerror_value_SYS_canservices_board_notfound, "SYS: the board was not found during the discovery phase.", "In par16 there is the board type"},
...
{   // very important: fill table with order of eOerror_value_HW_t
    ...
    {eoerror_value_HW_strain_saturation,       "HW: strain values saturation.", "In par16 there is the channel involved, in par64 there are the saturation counts for the last sec of both types (lower: LS 32bits, upper MS 32bits"},
...
{   // very important: fill table with order of eOerror_value_MC_t
    ...
    {eoerror_value_MC_joint_software_limit,  "MC: software limit reached. The joint position is outside its software boundaries. Output limited awaiting to be moved back inside safe positions.", "In par16 = ID of joint."}
...
{   // very important: fill table with order of eOerror_value_CFG_t
    //                 in case of holes, use {0, NULL}
    {eoerror_value_CFG_candiscovery_started, "CFG: CANdiscovery started the search for", "boards In par16: can2 map in 0xffff. In par64: can1 map in 0xffff000000000000, board type in 0x0000ff0000000000, req prot in 0x000000ffff000000, req fw in 0x0000000000ffffff. If prot and fw are zero, search on bus is not done"},
    {eoerror_value_CFG_candiscovery_ok, "CFG: CANdiscovery", "In par16: fake search in 0xf000, number of boards in 0x00ff. In par64: search time [ms] in 0xffff000000000000, board type in 0x0000ff0000000000, req prot in 0x000000ffff000000, req fw in 0x0000000000ffffff."},
    {eoerror_value_CFG_candiscovery_detectedboard, "CFG: CANdiscovery has detected ","In par16: board address in 0x000f. In par64: search time [ms] in 0xffff000000000000, board type in 0x0000ff0000000000, prot in 0x000000ffff000000, fw in 0x0000000000ffffff."},
    {eoerror_value_CFG_candiscovery_boardsmissing, "CFG: CANdiscovery cannot find ","In par16: target board type in 0xff00 and number of missing in 0x000f. In par64: search time [ms] in 0xffff000000000000, mask of missing addresses in 0x000000000000ffff"},
    {eoerror_value_CFG_candiscovery_boardsinvalid, "CFG: CANdiscovery detected","In par16: target board type in 0xff00 and number of invalid in 0x000f. In par64: each nibble contains 0x0 if ok, mask 0x1 if wrong type, mask 0x2 if wrong fw, mask 0x4 if wrong prot"}, 
    {eoerror_value_CFG_skin_ok, "CFG: EOtheSKIN can be correctly configured","In par16 num of patches, can mapping in par64 upper 32 bits, prot and vers in par64 lower 32 bits"}, 
    {eoerror_value_CFG_skin_failed_toomanyboards, "CFG: EOtheSKIN cannot be configured", "In par16: number of requested boards in 0xff00, max number in 0x00ff. In par64: mask of requested boards in 0x00000000ffff0000 (can2) and 0x000000000000ffff (can1)."}, 
    {eoerror_value_CFG_skin_failed_candiscovery, "CFG: EOtheSKIN cannot be configured. CANdiscovery fails","In par16 num of patches. In par64 from lsb to msb masks of: missing can1, can2, found but incompatible can1, can2"}, 
    {eoerror_value_CFG_strain_ok, "CFG: EOtheSTRAIN can be correctly configured", "can address in par16, prot and vers in par64 lower 32 bits. Byte 5 of par64: 0x01 is strain, 0x02 is strain2"}, 
    {eoerror_value_CFG_strain_failed_candiscovery, "CFG: EOtheSTRAIN cannot be configured. CANdiscovery fails", "can address in par16, prot and vers in par64 lower 32 bits. Byte 5 of par64: 0x01 is strain, 0x02 is strain2"}, 
    {eoerror_value_CFG_mais_ok, "CFG: EOtheMAIS can be correctly configured", "can address in par16, prot and vers in par64 lower 32 bits"}, 
    {eoerror_value_CFG_mais_failed_candiscovery, "CFG: EOtheMAIS cannot be configured. CANdiscovery fails", "can address in par16, prot and vers in par64 lower 32 bits"}, 
    ...
    {eoerror_value_CFG_mc_foc_failed_candiscovery_of_foc, "CFG: EOtheMotionController cannot be configured. CANdiscovery of 2foc boards fails. see CANdiscovery messages for more details",""},
    ...
    {eoerror_value_CFG_inertials3_ok, "CFG: EOtheInertial3 can be correctly configured", ""},
    {eoerror_value_CFG_inertials3_failed_toomanyboards, "CFG: EOtheInertial3 cannot be configured", "In par16: number of requested boards in 0xff00, max number in 0x00ff. In par64: mask of requested boards in 0x00000000ffff0000 (can2) and 0x000000000000ffff (can1)."}, 
    {eoerror_value_CFG_inertials3_failed_candiscovery, "CFG: EOtheInertial3 cannot be configured. CANdiscovery fails", "In par64 from lsb to msb masks of: missing can1, can2, found but incompatible can1, can2"}, 
    {eoerror_value_CFG_inertials3_failed_generic, "CFG: EOtheInertial3 cannot be configured. generic error", "More details in par16 and par64 and in source code"}, //recheck
    
    {eoerror_value_CFG_temperatures_not_verified_yet, "CFG: EOtheTemperatures service was not verified yet, thus it cannot start.", ""},
    {eoerror_value_CFG_temperatures_ok, "CFG: EOtheTemperatures can be correctly configured", ""}, 
    {eoerror_value_CFG_temperatures_failed_toomanyboards, "CFG: EOtheTemperatures cannot be configured", "In par16: number of requested boards in 0xff00, max number in 0x00ff. In par64: mask of requested boards in 0x00000000ffff0000 (can2) and 0x000000000000ffff (can1)."}, 
    {eoerror_value_CFG_temperatures_failed_candiscovery, "CFG: EOtheTemperatures cannot be configured. CANdiscovery fails", "In par64 from lsb to msb masks of: missing can1, can2, found but incompatible can1, can2"}, 
    {eoerror_value_CFG_temperatures_failed_generic, "CFG: EOtheTemperatures cannot be configured. generic error", "More details in par16 and par64 and in source code"}, 
    {eoerror_value_CFG_temperatures_changed_requestedrate, "CFG: EOtheTemperatures has changed the requested rate", "In par16 the requested (MSB) and the assigned (LSB)."}, 
    ...
    {eoerror_value_CFG_psc_ok, "CFG: EOthePSC can be correctly configured", "Can address in par16, prot and vers in par64 lower 32 bits"},
    {eoerror_value_CFG_psc_failed_candiscovery, "CFG: EOthePSC cannot be configured. CANdiscovery fails", "Can address in par16, prot and vers in par64 lower 32 bits"}, 
    ...
    {eoerror_value_CFG_psc_changed_requestedrate, "CFG: EOthePSC has changed the requested rate", "In par16 the requested (MSB) and the assigned (LSB)."}, 
    ...
    {eoerror_value_CFG_pos_ok, "CFG: EOthePOS can be correctly configured", "can address in par16, prot and vers in par64 lower 32 bits"},
    {eoerror_value_CFG_pos_failed_candiscovery, "CFG: EOthePOS cannot be configured. CANdiscovery fails", "Can address in par16, prot and vers in par64 lower 32 bits"},
    ...
    {eoerror_value_CFG_pos_changed_requestedrate, "CFG: EOthePOS has changed the requested rate", "In par16 the requested (MSB) and the assigned (LSB)."},
...
{   // very important: fill table with order of eOerror_value_ETHMON_t
    ...
    {eoerror_value_ETHMON_txseqnumbermissing, "ETH monitor: the board low level ETH detected a missing ropframe", "Expected sequence number in par64 and number of detected in par16"}, 
...
{   // very important: fill table with order of eOerror_value_AS_t
    //                 in case of holes, use {0, NULL}
    {eoerror_value_AS_arrayoftemperaturedataoverflow, "AS: cannot store rx can frames of temperature data, thus some readings will be lost", "In par16 there is frame.id and frame.size (in most significant nibble). In par64 there is the frame.data"}, 
